...
<template>
  <div class="flex items-center">
    <Listbox v-if="currentStyle === 'dropdown-right-top'" v-model="themeSetting" as="div"
      class="relative flex items-center">
      <ListboxLabel class="sr-only">
        {{ "change theme" }}
      </ListboxLabel>
      <ListboxButton type="button" title="change theme" class="transition-colors duration-300">
      ...
      <ListboxOptions
        ...
        <ListboxOption v-for="theme in availableThemes" :key="theme.key" :value="theme.key" :class="{
    ...
    <select v-if="currentStyle === 'select-box'" v-model="themeSetting"
